...
# Adpated from https://github.com/oduwsdl/tweetedat
...
def find_tweet_timestamp_pre_snowflake(tid):
    with open("TweetTimeline.txt", "r") as file_tweet_timeline:
...
def tidtotime(tid):
    tstamp = find_tweet_timestamp(tid)
    utcdttime = datetime.utcfromtimestamp(tstamp / 1000)
    #print(str(tid) + "   :  " + str(tstamp) + " => " + str(utcdttime))
    return str(utcdttime)
